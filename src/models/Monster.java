...
public class Monster extends Entity {
    ...
    /**
     * isPrimary = true  → this monster handles knocking, forced entry, dialogues
     * isPrimary = false → secondary monster, chases/attacks only, never knocks
     */
    ...
    /** Secondary monster 2 — spawns from the right edge */
    public void spawnFromRight() {
        worldX = 2600;
        worldY = gp.player.worldY + 200;
        state  = State.CHASING;
        animFrame   = 0;
        animCounter = 0;
    }

    /** Secondary monster 3 — spawns from the top-centre */
    public void spawnFromTop() {
        worldX = gp.player.worldX + 300;
        worldY = 100;
        state  = State.CHASING;
        animFrame   = 0;
        animCounter = 0;
    ...
    public void update() {
        ...
        int houseX = (gp.objectM.ObjHouse[0] != null)
            ? gp.objectM.ObjHouse[0].worldX + 75  : 1375;
        int houseY = (gp.objectM.ObjHouse[0] != null)
            ? gp.objectM.ObjHouse[0].worldY + 200 : 1050;

        double distToHouse = Math.sqrt(
            Math.pow(houseX - worldX, 2) + Math.pow(houseY - worldY, 2));

        // Only primary monster can force-enter the house
        if (isPrimary
                && (!playerOutside && !playerSafe
                    || !playerOutside && gp.hasOpenEntry() && distToHouse < KNOCK_RANGE)
                && !forceEnter
                && gp.interiorGraceTimer <= 0) {
            forceEnter = true;
            spawnInsideHouse();
        }
        ...
        if (playerOutside && !playerSafe) {
            ...
            if (distToHouse < KNOCK_RANGE) {
                ...
                if (isPrimary) {
                    // Primary monster knocks on the door
                    ...
                    // Secondary monsters linger around the house — no knocking
                    state = State.CHASING;
                    faceToward(houseX, houseY);
                }
    ...
    public void draw(Graphics2D g2) {
        ...
        BufferedImage frame;
        if (state == State.ATTACKING) {
            frame = slashFrames[currentDir][Math.min(animFrame, SLASH_FRAMES - 1)];
        } else {
