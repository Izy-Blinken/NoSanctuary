...
public class WinScreen {
    ...
    public void update() {
        if (alpha < 1f) {
            ...
            if (!inserted) {
                inserted = true;
                String savedUsername = gp.holder.getUsername();

                System.out.println("WinScreen: username from holder = " + savedUsername);
                System.out.println("WinScreen: completionSeconds = " + completionSeconds);

                if (savedUsername != null && !savedUsername.trim().isEmpty()) {
                    gp.dbConn.insert(savedUsername.trim(), completionSeconds);
                    System.out.println("Saved to DB successfully.");
                } else {
                    System.out.println("ERROR: username is null or empty — holder never received it.");
                }
            }
